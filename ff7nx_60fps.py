...
# The smooth-scripted wrapper's per-model records, after the player flag. Kept
# in the same BSS block the walk gate already grows, so enabling the feature
# adds no new allocation site -- only a larger bssSize.
SMOOTH_SCRATCH_OFF  = 0x2008
...
def _walk_gate_cave(D, orig, recover, cave, counter, flag_addr, hook,
    ...
    w = [
        ...
        0,                                         # 12: tbz w16,#0 -> DONE, or b smooth
        0x52800000 | (D & 0x1F),                   # 13: movz wD, #0 (odd: zero it)
        0,                                         # 14: b hook+4
    ]
    w[4]  = _tbz(16, 0, cave + 4 * 4, cave + 4 * 10)
    w[9]  = _b(cave + 4 * 9, cave + 4 * 14)
    # ONE word, and getting it wrong walks Cloud on the spot forever.
    #
    # THIS GATE LIVES INSIDE `field_update_single_model_position`. Words 10-13
    # are the scripted/NPC path: keep the native step on an even tick, zero it
    # on an odd one, so scripted movement advances once per tick PAIR instead
    # of twice. That is the right thing when every field frame calls the
    # function.
    #
    # The smooth wrapper (ff7nx_smooth.py) changes that premise: it calls the
    # function ONCE per pair and replays the result on the other frame. Leave
    # the gate in and the two compose catastrophically -- the wrapper's single
    # call lands on whichever parity the model started moving on, and if that
    # is the odd one the gate zeroes every step it ever computes. The model
    # never moves, the arrival test never fires, and the scene never ends.
    # Fifty-fifty on when the move began, which is exactly why some cutscenes
    # looked right and the weapon-shop bedroom hung.
    #
    # So with the wrapper on, the scripted path passes the full stock step
    # through every time, and one-step-per-pair becomes solely the wrapper's
    # job -- which it does by not making the call at all, rather than by
    # making it and discarding the answer.
    w[12] = (_b(cave + 4 * 12, cave + 4 * 14) if smooth
             else _tbz(16, 0, cave + 4 * 12, cave + 4 * 14))
    w[14] = _b(cave + 4 * 14, hook + 4)
    return w


def build_walk_gate_caves(text, ro_base, segs, data, verify_only, log,
                          nso_path=None, smooth=False):
    ...
    # exact size and shape they were confirmed with on hardware, so enabling
    # this moves two words of .text and nothing else.
    # ------------------------------------------------------------------
    # SMOOTH SCRIPTED MOVEMENT -- the call-site wrapper (ff7nx_smooth.py).
    #
    # This used to HALVE THE STEP here, and it froze Cloud's backwards walk at
    # Shinra HQ: the arrival test compares a squared distance against a
    # threshold built from the model's speed, and halving one without the
    # other stops them agreeing. README-35 has the disassembly.
    #
    # What replaces it does not touch the step at all. It wraps the two calls
    # FFNx wraps, runs the real update once per tick pair at full stock speed
    # from an exact stock position, and interpolates only what is left behind
    # for the frame in between. The five gate caves below are byte-identical
    # whether this is on or off -- the only thing enabling it changes is two
    # `bl` targets and some reclaimed padding.
    # ------------------------------------------------------------------
    if smooth and not verify_only:
        import a64
        import ff7nx_cave, cave_space, nxmap
        import ff7nx_smooth as SW
        if not nso_path:
            raise SystemExit('ABORT  %s needs the padding-hole map; '
                             'pass nso_path.' % SMOOTH_SCRIPTED_ENV)
        for hk, want, nm in ((SW.MOVE_HOOK, SW.MOVE_ORIG, 'scripted movement'),
                             (SW.COLL_HOOK, SW.COLL_ORIG, 'arrival test')):
            cur, = struct.unpack('<I', text[hk:hk + 4])
            if cur != want:
                raise SystemExit(
                    'ABORT  smooth-scripted: the %s call site at +0x%X reads '
                    '%08X, expected %08X.\n       Something else patched it, '
                    'or this is not the build these addresses came from.'
                    % (nm, hk, cur, want))
        _m = nxmap.Main(nso_path)
        pool = ff7nx_cave.HolePool(
            text, starts=set(_m.arm_starts),
            named=cave_space.named_targets(_m.img[cave_space.RODATA:]))
        scratch = base_ctr + SMOOTH_SCRATCH_OFF
        # emit_laid_out is not used here, and the reason is worth stating: it
        # returns only the ENTRY address and the placed map, and `link` pads
        # the gaps between holes with bridging branches. So the address of the
        # cave's i'th INSTRUCTION is not `sorted(placed)[i]` -- the bridges sit
        # in between. This cave has two entry points, and finding the second
        # one needs the index->address map that `slots` produces, so the
        # placement is done a step at a time to keep hold of it.
        try:
            probe, _ = SW.build(lambda i: 4 * i, scratch, counter)
            runs = pool.take(len(probe), span=0x80000)
            addrs = ff7nx_cave.slots(runs, len(probe))
            words, entries = SW.build(lambda i: addrs[i], scratch, counter)
            if len(words) != len(probe):
                raise SystemExit(
                    'ABORT  smooth-scripted: the builder emitted %d words on '
                    'the real layout and %d on the probe' % (len(words),
                                                             len(probe)))
            placed = ff7nx_cave.link(runs, words)
        except ff7nx_cave.NoRoom as exc:
            raise SystemExit('ABORT  smooth-scripted: %s' % exc)
        for va, word in placed.items():
            if struct.unpack_from('<I', text, va)[0] != 0:
                raise SystemExit('ABORT  smooth-scripted wanted padding at '
                                 '+0x%X and it is not zero' % va)
            struct.pack_into('<I', text, va, word)
            _WALK_SMOOTH_WORDS[va] = word
        # Both hooks are `bl` and the wrapper ends in `ret`, so these stay
        # calls -- the caller's own code after the call site is untouched.
        for hk, key, nm in ((SW.MOVE_HOOK, 'move', 'scripted movement'),
                            (SW.COLL_HOOK, 'coll', 'arrival test')):
            tgt = addrs[entries[key]]
            hookword = a64.bl(hk, tgt)
            struct.pack_into('<I', text, hk, hookword)
            # recorded like the padding words, so the .text diff audit and the
            # NSO self-check account for them instead of reporting them as
            # changes nobody claimed
            _WALK_SMOOTH_WORDS[hk] = hookword
            log('      smooth scripted movement: %s call +0x%06X -> wrapper '
                '0x%06X' % (nm, hk, tgt))
        log('      smooth scripted movement: %d words across %d hole(s), '
            'scratch at module +0x%X (%d bytes)'
            % (len(placed), len(pool.used), scratch, SW.SCRATCH_BYTES))
        _BSS_GROW = max(_BSS_GROW, SMOOTH_SCRATCH_OFF + SW.SCRATCH_BYTES)
        pool.used = []
...
# Words the smooth-scripted helpers wrote into reclaimed padding. Recorded so
# the .text diff audit can account for every one of them instead of refusing
# the build -- the same contract the padding caves in _place_padding_caves use.
_WALK_SMOOTH_WORDS = {}
...
# ==========================================================================
# Three times now a change has been written, tested and delivered, and then a
# build has gone out without it -- once because an older ff7nx_60fps.py was
# still in place, twice because an option defaulted off and nobody said so.
# Every one of those cost a hardware test and a round of "it did not work".
#
# The build log named the groups that WERE on. It never named the ones the
# running code KNOWS ABOUT but had switched off, so "absent" and "off" printed
# identically -- and a stale file printed the same as a correct one.
#
# This prints every optional feature the running code supports together with
# its state. Two things follow, and both are the point:
#
#   * an option that is off says so, by name, instead of saying nothing;
#   * an option the code has never heard of CANNOT PRINT AT ALL, so a stale
#     ff7nx_60fps.py is visible at a glance rather than after a test cycle.
#
# Anything added here must be listed the moment it exists, off or on. A
# feature that only prints when enabled reintroduces exactly the ambiguity
# this exists to remove.
def capability_banner(want, a):
    """One line naming every optional feature and whether it is on."""
    fps = getattr(a, 'limiter_fps', None)
    bits = [
        ('analog-360',      ANALOG_GROUP in want),
        ('no-autorun',      AUTORUN_GROUP in want),
        ('no-cheats',       NOCHEATS_GROUP in want),
        ('battle-text',     'battle-text' in want),
        ('smooth-scripted', 'on (60 FPS set)' if smooth_scripted()
                            else 'OFF (overridden)'),
        ('limiter-fps',     ('%d' % fps) if fps and fps != 60 else False),
    ]
    out = []
    for name, state in bits:
        if state is True:
            out.append('%s=on' % name)
        elif state is False:
            out.append('%s=OFF' % name)
        else:
            out.append('%s=%s' % (name, state))
    return ('options: ' + '  '.join(out)
            + '\n         (a feature MISSING from this line means '
              'ff7nx_60fps.py is out of date)')
...
def patch_nso(data, patches, verify_only=False, log=print, extra=(),
    ...
    build_walk_gate_caves(text, ro_base, segs, data, verify_only, log,
                          nso_path=nso_path, smooth=smooth_scripted())
    ...
    for c in dispatch:
        ...
        if list(got) != list(want):
            ...
            log('  !! cave for %s differs from what was asked for at word(s) %s'
                % (c['label'], bad[:8]))
            ok = False
    for va, word in _WALK_SMOOTH_WORDS.items():
        cur, = struct.unpack('<I', raw[0][va:va + 4])
        if cur != word:
            log('  !! smooth-scripted helper word at +0x%X reads %08X, '
                'wanted %08X' % (va, cur, word))
            ok = False
    for c in NSO_CAVES:
...
def main():
    ...
    if stray:
        ...
        padwords = set(_WALK_SMOOTH_WORDS)
        for c in dispatch:
