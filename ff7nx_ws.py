...
def apply_module(sdout, dump, log=lambda *_: None, produced=()):
    ...
    log('    FIELD: the ~32-unit bar along the BOTTOM should be gone. The')
    log('      top was already covered.')
    ...
    log('    NO LONGER TRUE: "the margins are not cleared". They are, if')
    log('      Display extras -> Clear the 16:9 margins is on -- see the')
    log('      "clearing the field frame" pass further down this log.')
    return produced_now
