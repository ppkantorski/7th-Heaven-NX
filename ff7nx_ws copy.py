...
def apply_module(sdout, dump, log=lambda *_: None, produced=()):
    ...
    WHAT IT SHIPS
    -------------
    The 2D half, and only the 2D half. Three parts, all in-place words:
      ...
      B. The TLVERTEX ortho, 0..640 -> -106.75..747.25 (three words, above).
      C. `ff7nx_fieldwide`'s five parallax clip/wrap words, so layers 3 and
         4 do not cull at the old 320-unit boundary once the frame is wider.

    A and B are one change: A alone stretches (attempt 1), B alone squashes
    the picture into three quarters of a 4:3 target. Together the scale is
    2.250 px/unit before and 2.248 after -- no stretch, by construction.

    WHAT IT DOES NOT SHIP, AND WHAT THAT WILL LOOK LIKE
    ---------------------------------------------------
    The 3D half. Models, the battle scene and the world map do not go
    through the TLVERTEX ortho; they take the branch at +0x10DA098 and use
    the game's own projection composed with `d3dviewport_matrix`. Nothing
    here touches that, so **3D will be horizontally stretched by 854/640 =
    1.33x** while every 2D thing around it is correct.

    That is the expected outcome, it is stated in advance, and it is the
    whole point: it is a single-variable measurement. If 2D comes out right
    and only models are wide, the model in README-47 is confirmed and the
    remaining work is the 3D half (README-47 §3), which is bounded and
    known. If 2D comes out wrong, the model is wrong and this cost one
    build to find out instead of five.

    The v7 patch set (`ff7nx_framing` + the flevel camera transform) is NOT
    shipped here any more. README-46 §2 established why it could not have
    worked: its cave sat inside `gfx_drv_setviewport` and changed a value
    that +0x10D9D70 recomputes before anything is drawn.
    """
    ...
    try:
        ...
        applied += nso_patcher.apply_spec(nso, projection_2d_spec())
        if three_d:
            # `_41` stays stock, so the span is 0..854 and the widened FIELD
            # viewport lands exactly on it.
            applied += nso_patcher.apply_spec(nso, {
                'name': '3D viewport matrix: game_w 640 -> 854',
                'patches': [dict(p) for p in GAME_W_PATCHES]})
            applied += nso_patcher.apply_spec(nso, {
                'name': 'field mode-2 group widened to 854/427',
                'patches': [dict(p) for p in FIELD_MODE2_PATCHES]})
            applied += nso_patcher.apply_spec(nso, {
                'name': 'field vertical uncrop 448 -> 480',
                'patches': [dict(p) for p in UNCROP_PATCHES]})
        else:
            applied += nso_patcher.apply_spec(nso, {
                'name': '2D ortho origin -> -106.75',
                'patches': [dict(ORTHO_ORIGIN_PATCH)]})
        applied += nso_patcher.apply_spec(nso,
                                          ff7nx_fieldwide.parallax_spec())
    ...
    left, right = ortho_2d(True, shift_origin=not three_d)
    for h in (720, 1080):
        log('  %dp: render target %d -> %d wide, 2D projection %.2f..%.2f '
            '(%.2f game units), %.3f -> %.3f px/unit'
            % (h,
               ff7nx_widescreen.logical_width(h, False) * 3 // 2,
               ff7nx_widescreen.logical_width(h, True) * 3 // 2,
               left, right, right - left,
               ff7nx_widescreen.logical_width(h, False) * 1.5 / GAME_W_43,
               ff7nx_widescreen.logical_width(h, True) * 1.5 / (right - left)))
    log('')
    log('  WHAT TO LOOK FOR, and please report it in these terms:')
    if three_d:
        log('    The FIELD should fill the screen in BOTH directions --')
        log('      the 48px bars top and bottom come from its 448-of-480')
        log('      viewport and are lifted by the uncrop words.')
        log('    FIELD should now be real widescreen: full width, NOT')
        log('      stretched, showing the background Cosmos repainted past')
        log('      the old 4:3 crop. Characters should sit ON the scenery.')
        log('    BATTLE and MENUS should be the right SHAPE (not stretched)')
        log('      but sitting about 160px LEFT of centre at 720p. That is')
        log('      the one known gap: their viewport still starts at x=0 in')
        log('      an 854-wide frame. README-47 §4 is the cave that fixes it.')
        log('    If anything is STRETCHED, game_w did not take -- that is a')
        log('      different failure from "off-centre" and points elsewhere.')
    else:
        log('    2D -- backgrounds, menus, text -- correct and centred;')
        log('      the field only paints the middle 4:3, so the margins are')
        log('      black. 3D stretched 1.33x. Use ws-3d instead.')
    return [] if built else [dest]
